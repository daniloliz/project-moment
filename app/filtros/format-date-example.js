(function (angular) {
  ...
  .filter('formatDatesExample', ['$window', function($window) {
    return function (date, locale) {
      if (date) {
        return $window.moment.tz(date, locale).format('dddd');
      } else {
        return date;
      }
    };
  ...
  .filter('formatRelativeTime', ['$window', function($window) {
    return function (date, locale) {
      if (date) {
        return $window.moment.tz(date, locale).endOf('day').fromNow();
      } else {
        return date;
      }
    };
  ...
  .filter('formatCalendarTime', ['$window', function($window) {
    return function (date, locale) {
      if (date) {
        return $window.moment.tz(date, locale).subtract(10, 'days').calendar();
      } else {
        return date;
      }
    };
  ...
  .filter('formatLocale', ['$window', function($window) {
    return function (date, locale) {
      if (date) {
        return $window.moment.tz(date, locale).format('LTS');
      } else {
        return date;
      }
    };
